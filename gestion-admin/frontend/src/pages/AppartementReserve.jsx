...
const AppartementReserve = () => {
  ...
  const fetchReservations = async () => {
    try {
      const res = await axios.get("/api/reservation/show/appartement");
  ...
  const handleInputChange = (e, isNew = false) => {
    ...
    if (isNew) {
      ...
      // Recalculer le total si days_count change et item_id déjà connu
      if (name === "days_count" && updated.item_id && updated.price) {
        updated.total_price = updated.days_count * updated.price;
      }
      setNewReservation(updated);
    } else {
      const updated = { ...currentProduct, [name]: value };
      if (name === "days_count" && currentProduct.price) {
  ...
  const handleAddReservation = async (e) => {
    e.preventDefault();
    try {
      // Obtenir l'ID et le prix à partir du nom
      const res = await axios.get(`/api/appartement/name/${newReservation.itemName}`);
      const { _id, price } = res.data;

      const reservationData = {
        ...newReservation,
        item_id: _id,
        total_price: newReservation.days_count * price,
        price,
        available: false,
      };
  ...
  const handleUpdateProduct = async (e) => {
    ...
    try {
      const updated = {
        ...currentProduct,
        total_price: currentProduct.days_count * currentProduct.price,
      };
  ...
  return (
    <div className="table-container">
      <h2>Réservations des Appartements</h2>
        ...
        <div className="modal">
          <div className="modal-content">
            ...
            <form onSubmit={handleAddReservation}>
              ...
              <input name="itemName" placeholder="Nom de l'appartement" value={newReservation.itemName} onChange={(e) => handleInputChange(e, true)} required />
              ...
              <input name="days_count" type="number" placeholder="Nombre de jours" min={1} max={31} value={newReservation.days_count} onChange={(e) => handleInputChange(e, true)} />
              <input name="persons_count" type="number" placeholder="Nombre de personnes" min={1} max={31} value={newReservation.persons_count} onChange={(e) => handleInputChange(e, true)} />
